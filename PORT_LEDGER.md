...
| 8 | Gallery post-process hangs when the WS drops (mobile background throttle over the tunnel) — chunk's `queuePrompt` waited only on WS `execution_success`; job finishes, result in `/history`, promise never resolves → stuck on "preparing chunk 1/N" | `v1.21.4` | `24458d0` | 2026-09-01 | web→node | Diagnosed web-side from an iPhone. `queuePrompt`'s fresh-submit path now runs the same 5s `/history` poll the reattach path already had (node added it as defence-in-depth — its WS is usually stable). **Q2 done** (`514b6dc`): `opts.onPoll` fires each poll tick while running → gallery readout shows "still working (connection quiet)…", main render status likewise, so the label isn't frozen while only the poll reports. |
