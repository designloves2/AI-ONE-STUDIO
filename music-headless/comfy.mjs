// comfy.mjs — ComfyUI HTTP client for the headless MusicMaker generator. Node 20+
// (global fetch). Ported from krea2-headless/comfy.mjs — identical submit/poll/download,
// only extractOutputs is taught about SaveAudioAdvanced's `audio` result.
//
...
export function makeClient(comfyConfig, { apiPrefix = "" } = {}) {
  ...
  async function getJson(path) {
    ...
    if (!r.ok) { const err = new Error(`GET ${path} -> ${r.status}`); err.stage = "config"; throw err; }
    return r.json();
  }
  ...
  async function postJson(path, body) {
    ...
    if (!r.ok) {
      ...
      err.stage = "submit"; err.body = data; throw err;
    }
    return data;
  }

  const cfg = () => getJson(`${apiPrefix}/config`).catch((e) => { if (e.stage === "auth") throw e; return {}; });
  const models = () => getJson(`${apiPrefix}/models`).catch(() => ({}));
  ...
  async function submitGraph(graph, { pollMs = 900, onPoll } = {}) {
    ...
    if (data.error) {
      const detail = data.node_errors && Object.keys(data.node_errors).length
        ? ` (${Object.keys(data.node_errors).join(", ")})` : "";
      const err = new Error(`${data.error.message || "queue failed"}${detail}`);
      err.stage = "submit"; err.body = data; throw err;
    }
    const promptId = data.prompt_id;
    if (!promptId) { const err = new Error("no prompt_id in /prompt response"); err.stage = "submit"; throw err; }

    const deadline = Date.now() + timeoutMs;
    for (;;) {
      if (Date.now() > deadline) {
        const err = new Error(`timed out after ${Math.round(timeoutMs / 1000)}s waiting for ${promptId}`);
        err.stage = "timeout"; err.promptId = promptId; throw err;
      }
      await new Promise((res) => setTimeout(res, pollMs));
      let hd;
      try { hd = await getJson(`/history/${promptId}`); }
      catch (e) { if (e.stage === "auth") throw e; continue; }
      ...
      if (msgTypes.includes("execution_interrupted")) {
        ...
        err.stage = "interrupted"; err.promptId = promptId; throw err;
      }
      if (status.status_str === "error" || msgTypes.includes("execution_error")) {
        const msg = (status.messages || [])
          ...
          .filter(Boolean).join(" | ") || "execution error";
        const err = new Error(msg);
        err.stage = "generate"; err.promptId = promptId; err.history = entry; throw err;
      }
      if (status.completed) return { promptId, outputs: entry.outputs || {}, status };
      onPoll?.(promptId);
    }
  }
  ...
  async function downloadOutput(outFile, destDir) {
    ...
    if (!r.ok) { const err = new Error(`GET /view -> ${r.status} for ${outFile.filename}`); err.stage = "download"; throw err; }
...
export function extractOutputs(outputsByNode, saveNode) {
  ...
  const files = out.audio || out.audios || out.images || [];
  return files.map((f) => ({
    type: "audio",
    filename: f.filename,
