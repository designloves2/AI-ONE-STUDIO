...
import { getComfyBase, getComfyWsBase } from "./comfyBase";
...
export async function comfyFetch(path: string, opts?: RequestInit) {
  // credentials: "include" — external access is behind Cloudflare Access; the CF_Authorization
  // cookie (SameSite=None) only rides cross-origin fetches with credentials included.
  return fetch(`${COMFY_BASE}${path}`, { ...opts, credentials: "include" });
}
