...
| § | item | node | web | verified | origin | notes |
|---|---|---|---|---|---|---|
| 1 | Enhance-result review: 3 apply modes (One Prompt / Auto Split / Use selected) chosen after the model answers, centred caption over the buttons, mode block above Discard/Apply. Manual mode = click cards to pick; selected 2px brand border, off = grey border + inset black scrim. Shot-count mismatch warning for target "all". | `v1.22.0` | — | 2026-09-01 | user | Cards always preview; only apply method chosen last. `reviewSel` seeded all-ticked. |
| 2 | `parseBrief`: broadened `TAIL_RE` to catch the structured audio section (`overall_soundscape:` / `non_diegetic_music:` + markdown/underscore/hyphen variants), added `ECHO_RE` + `stripEcho()` to drop the model's trailing instruction / vision-analysis echo from every block. | `v1.22.0` | — | 2026-09-01 | user | Was only matching `Ambient sound:` / `Music:` → structured briefs glued the tail onto the last shot. |
