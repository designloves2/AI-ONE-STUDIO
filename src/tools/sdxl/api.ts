// api.ts — SDXL 백엔드 REST 호출 이식.
// 원본 근거: ComfyUI-TJ_NODE_STUDIO_ONE/web/sdxl/api_sdxl.js (API="/sdxl_one")
...
export async function getModels(): Promise<{ checkpoints: string[]; unets: string[]; text_encoders: string[]; vaes: string[]; loras: string[] }> {
  try {
    return await jsonFetch(`${API}/models`);
  } catch {
    return { checkpoints: [], unets: [], text_encoders: [], vaes: [], loras: [] };
  }
}

export async function getESRGANModels(): Promise<{ models: string[] }> {
  try {
    return await jsonFetch(`${API}/esrgan_models`);
  } catch {
    return { models: [] };
  }
}
