...
| 8 | Gallery chunked Upscale/Interpolate hangs on iOS — each chunk's `queuePrompt` waited only on the WS `execution_success`, which iOS Safari drops on a long backgrounded tab over the tunnel; job finishes, result in `/history`, promise never resolves → stuck on "preparing chunk 1/N" | n/a | `24458d0` | 2026-09-01 | web→node | Web-only fix: `queuePrompt`'s fresh-submit path now runs the same 5s `/history` poll the reattach path already had. Q2 (poll heartbeat for the frozen mobile progress label) still open — cosmetic. |
| 9 | Upscale/Deblur chunk size — per-method duration rule, not byte budget | — | `431be92` | 2026-09-01 | web→node | User rule: **RTX VSR / RTX deblur** = whole file `<15s`, 15s chunks `>=15s`. **Upscale model** = whole file `<10s`, 5s chunks `>=10s`. deblur+model → model rule. Interpolate keeps byte-budget (no rule given). Web: `runPost` gained `chunkPlan(durationSec)=>chunkSeconds`; byte-budget stays as the video_info-unreadable fallback. **Node: mirror in §16.** |
| 8 | Stitch mp4 faststart (already row 7) — node also: `queuePrompt` gained a 5s `/history` poll so a dropped websocket on a backgrounded mobile tab doesn't hang the post-process loop forever; post-process chunking is now **duration-based** (RTX/RTX-deblur <15s whole / ≥15s 15s chunks; model <10s whole / ≥10s 5s chunks; interpolate keeps byte budget) | `v1.21.4`+`v1.21.5` | `24458d0`+`431be92` | 2026-09-01 | web→node | Chunk-hang + web queue fix diagnosed web-side. Chunk rule settled by the user, both repos matched. Node's `/history` poll is defence-in-depth (its WS is usually stable). |
