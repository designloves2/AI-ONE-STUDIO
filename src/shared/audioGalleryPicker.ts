// audioGalleryPicker.ts — cross-tool audio picker for audio-upload slots.
// 원본 근거: ComfyUI-TJ_NODE_STUDIO_ONE/web/shared/ui_audio_gallery_picker.js (node be7b401).
//
// Image slots can pick from any image tool's gallery; audio had no equivalent. This overlay
 ...
 */
export function openAudioGalleryPicker(onPick: (name: string) => void, copyApi = "/minimax_h3_one") {
  ...
  async function pick(track: any, rowEl: HTMLElement) {
    if (picking) return;
    picking = true;
    stopPreview();
    rowEl.style.opacity = "0.5";
    try {
      const r = await fetchApi(`${copyApi}/copy_to_input`, {
  ...
  async function load() {
    ...
    try {
      ...
      statusEl.textContent = tracks.length
        ...
        : "No tracks in the MusicMaker playlist yet — make one in MusicMaker first.";
    } catch (e: any) {
