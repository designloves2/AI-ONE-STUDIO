...
| 8 | Gallery chunked Upscale/Interpolate hangs on iOS — each chunk's `queuePrompt` waited only on the WS `execution_success`, which iOS Safari drops on a long backgrounded tab over the tunnel; job finishes, result in `/history`, promise never resolves → stuck on "preparing chunk 1/N" | n/a | `24458d0` | 2026-09-01 | web→node | Web-only fix: `queuePrompt`'s fresh-submit path now runs the same 5s `/history` poll the reattach path already had. Open (node): Q1 raise `CHUNK_BUDGET_BYTES` (1.25 GB) so 8s/~1.3MP clips stop chunking into 3-4 pieces? Q2 poll heartbeat for the frozen mobile progress label? |
