...
// FormData / Blob). Ported from src/tools/*/api.ts + comfyClient.ts, minus the WebSocket path —
// a plain /history poll is all a server-side caller needs.
//
...
export function makeClient(comfyConfig, { apiPrefix = "" } = {}) {
  ...
  const getJsonSafe = (path) => getJson(path).catch((e) => { if (e.stage === "auth") throw e; return {}; });
  const cfg = () => getJsonSafe(`${apiPrefix}/config`);
  const models = () => getJsonSafe(`${apiPrefix}/models`);
  const seedvr2Models = () => getJsonSafe(`${apiPrefix}/seedvr2_models`);
  ...
  return { baseUrl, clientId, req, getJson, postJson, uploadImage, cfg, models, seedvr2Models, viewUrl, submitGraph, downloadOutput };
}

/** Pull the image outputs out of a SaveImage node result. */
