...
| 7 | Stitched mp4 won't play on iOS Safari — `moov` atom at end of file | — | n/a | 2026-09-01 | web→node | Found on iPhone (studio.tjtj.cloud). Verified by atom dump: individual clips = `ftyp/moov/free/mdat` (faststart, from SaveVideo), stitched = `ftyp/free/mdat/moov`. Desktop buffers whole file + seeks back; iOS refuses HTTP playback without front `moov`. **Node fix**: add `-movflags +faststart` to every branch of `mmh3_stitch`'s ffmpeg cmd (`-c copy` + the 3 libx264 branches), before `out_path`. Hits One-Take auto-stitch, Extend, manual Combine. Check upscale/deblur/interpolate routes too. Web: nothing to do. |
