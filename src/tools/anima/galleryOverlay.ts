// galleryOverlay.ts — Anima 이미지 갤러리 오버레이. Z-Image와 동일 패턴, SEND_TARGETS만 다름.
...
const SEND_TARGETS: { mode: string; field: string; label: string }[] = [
  { mode: "inpaint", field: "inpaintImage", label: "→ Inpainting" },
  { mode: "anycontrol", field: "anyControlImage", label: "→ Any Control" },
  { mode: "depthcontrol", field: "depthControlImage", label: "→ Depth Control" },
];
...
export function createGalleryOverlay(state: { saveSubfolder: string }, onReuse: (meta: any) => void, onSendTo: (mode: string, field: string, filename: string) => void) {
  ...
  topRow.appendChild(el("div", { text: "🖼 Gallery — Anima", style: { color: "#fff", fontSize: "14px", fontWeight: "700", flex: "1" } }));
  ...
  function thumb(img: GalleryImage, idx: number) {
    ...
    if (selectMode) {
      ...
      const star = el("button", { text: img.favorite ? "★" : "☆", type: "button", style: { position: "absolute", zIndex: "3", top: "2px", right: "2px", background: "rgba(0,0,0,0.15)", color: img.favorite ? BRAND : "#fff", border: "none", borderRadius: "8px", textShadow: "0 0 3px rgba(0,0,0,0.95)", width: "18px", height: "18px", fontSize: "13px", transform: "translateY(-2px)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", lineHeight: "1", padding: "0" } });
