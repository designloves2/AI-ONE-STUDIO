// api.ts — Flux2 Klein 백엔드 REST 호출 이식.
// 원본 근거: ComfyUI-TJ_NODE_STUDIO_ONE/web/klein/api_klein.js (API="/flux_klein")
...
// 원본 api_klein.js는 존재하지 않는 /flux_klein/free_memory를 호출해 항상 조용히 실패한다
// (nodes.py에 해당 라우트가 없음 — Z-Image에만 전용 free_memory 핸들러가 있다). 실제로 동작하도록
// Krea2와 동일하게 ComfyUI 코어의 범용 /free 엔드포인트를 사용한다.
