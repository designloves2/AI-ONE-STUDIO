...
// 원본 근거: one_node_qwen21.js는 klein/ui_prompt_templates.js와 동일한 BUILT_IN 카테고리
// (EDIT: ANGLES/RELIGHT/STYLES/OTHER, PAINT: SKETCH/COLLAGE/INPAINT/OUTPAINT)를 그대로 재사용한다
// — qwen2511 TS 포트가 이미 이 BUILT_IN 테이블을 1:1 이식해뒀으므로 EDIT/PAINT 항목을 그대로
// 가져온다(이전에는 이 테이블 자체를 빼먹고 커스텀 템플릿만 남겨서 실제 노드에 있는 프리셋이
// 하나도 안 보이는 버그였다). T2I/I2I/UPSCALE은 2511과 동일하게 빌트인 항목이 없다(Faceswap은
// 2.1에 없는 모드라 제외).
...
// qwen2511/promptTools.ts의 BUILT_IN.edit / BUILT_IN.inpaint를 그대로 복사 — state.mode는
// "inpaint" 하나만 있고(paintSubMode로 Inpaint/Outpaint 구분), 2511도 동일하게 SKETCH/COLLAGE/
// INPAINT/OUTPAINT 4개 카테고리를 서브탭 구분 없이 한 목록에 같이 보여준다.
