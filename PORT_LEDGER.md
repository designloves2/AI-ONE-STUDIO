...
| 10 | Post-process meta reflects the **output**, not the source — upscale changes w/h, interpolation changes frames+fps, but `runPost` copied `{...v.meta}` verbatim so the "i" info showed the source size | — | `4a9d683` | 2026-09-01 | web→node | Found on iPhone: 2× upscale of a 1440×960 clip → real 2880×1920, meta said 1440×960. Web: `runPost` probes the finished output (`getVideoInfo`) → overwrites w/h/frames/fps/durationSeconds, drops the source `elapsedSec`, adds `postProcess`+`sourceW`/`sourceH`; `metaInfoLines` shows "⚙ upscale (from WxH)" + "N frames @ Mfps". Prompt/seed/pipeline still carried for Reuse (§5). Existing files keep stale meta. **Node: same gap in §16 `runPost`, mirror when back.** |
