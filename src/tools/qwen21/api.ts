// api.ts — QWEN IMAGE 2.1 백엔드 REST 호출 이식.
// 원본 근거: web/qwen21/api_qwen21.js (API="/qwenimage21_one")
...
// 드로잉 오버레이(maskDraw.ts)의 캔버스 합성 결과(Blob)를 업로드 — 원본은 별도 라우트 없이
// 동일한 /upload/image를 그대로 재사용한다 (api_qwen21.js의 uploadAnnotationBlob).
export async function uploadAnnotationBlob(blob: Blob, filenameHint = "annotation.png"): Promise<string> {
  const fd = new FormData();
  fd.append("image", blob, filenameHint);
  fd.append("subfolder", "");
  fd.append("type", "input");
  const r = await comfyApi.fetchApi("/upload/image", { method: "POST", body: fd });
  const d = await r.json();
...
// 원본 상단바 Unload 버튼은 API 접두사 없이 코어 /free를 직접 호출한다 (Klein/2511과 동일 패턴).
