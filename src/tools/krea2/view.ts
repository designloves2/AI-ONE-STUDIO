...
export function renderKrea2(root: HTMLElement) {
  ...
  // node's main-screen header restyle (Krea2-era + 8938d00 for the other 5 tools): icon-only
  // buttons → solid brand-purple bg, white text, text labels.
  ...
  // Hermes agent job file download — krea2 is in scope (job.json header button = krea2 +
  ...
  const promptExpandOv = createPromptEditPopup({
    ...
    viewUrl: (filename) => api.viewUrl(filename),
    llm: llmState,
  ...
  // Moved from a left-panel button into the prompt header (agentJsonHdrBtn above) — krea2 is
  // in scope for the header job.json button (zimage too; klein explicitly excluded).
