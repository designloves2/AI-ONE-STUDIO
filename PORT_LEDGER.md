...
| 6 | Reuse/clip-meta reproduces the turbo config, not just `turboMode` — `metaForVideo` now writes pddFile/pddFileReference/pddNfe/turboLora*/turboSteps/slaTurboSteps/slaTurboLora/scheduler/denoise/shift, `reuseAll` restores them | `v1.21.2` | — | 2026-09-01 | web→node | Web session found it while porting §1.21.1. Same class as §4: `turboMode="pdd"` restored without the file → `effectiveTurbo` falls back → normal steps. `reuseAll` already had a dead turbo-LoRA restore branch. |
