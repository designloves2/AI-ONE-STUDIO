...
| 7 | Stitched mp4 won't play on iOS Safari — `moov` atom at end of file | `v1.21.3` | n/a | 2026-09-01 | web→node | Found on iPhone (studio.tjtj.cloud). Verified by atom dump: individual clips = `ftyp/moov/free/mdat` (faststart, from SaveVideo), stitched = `ftyp/free/mdat/moov`. Desktop buffers whole file + seeks back; iOS refuses HTTP playback without front `moov`. Node `v1.21.3` added `-movflags +faststart` to all three `mmh3_stitch` ffmpeg branches (stream-copy concat + trim/overlap + audio-override); also covers the chunked upscale/deblur/interpolate joins (same concat-demuxer branch via `stitchClips(..., 0, 0)`). Node-verified: re-stitch now writes `ftyp → moov → …`. Web: nothing to do. **Old stitched files stay broken — re-run any stitch/Extend to get a playable one.** |
| 7 | Stitched mp4 `moov` after `mdat` → won't play on iOS Safari over HTTP. `-movflags +faststart` added to all 3 `mmh3_stitch` ffmpeg branches (covers One-Take stitch, Extend, manual Combine, chunked post-process joins) | `v1.21.3` | n/a | 2026-09-01 | web→node | `/stitch` route is node-only. Diagnosed web-side from the atom order (user hit it on iPhone via the cloudflared tunnel). Old stitched files stay broken; re-run to fix. |
