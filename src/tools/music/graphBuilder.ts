// graphBuilder.ts — MusicMaker ONE STUDIO (TJ) workflow graph builders.
// 원본 근거: ComfyUI-TJ_NODE_STUDIO_ONE/web/music/graph_builder_music.js — 1:1 이식
// (노드 id 문자열, class_type, 입력 필드명, dotted key "format.quality"까지 원본과 동일).
...
// SaveAudioAdvanced: format="mp3" + "format.quality"="V0". flac has none.
function saveAudioNode(audioLink: any, prefix: string, state: any) {
  const fmt = (["flac", "mp3", "opus"].includes(state.format) ? state.format : "flac");
  const inputs: any = { audio: audioLink, filename_prefix: prefix, format: fmt };
...
function withLoraChain(prefix: string, modelLink: any, loras: any[]): { graph: Graph; modelOut: any } {
  const graph: Graph = {};
  let out = modelLink;
...
export function clampDuration(seconds: any, { min = 1, max = 300 }: { min?: number; max?: number } = {}) {
...
function captionWithHints(caption: string, state: any, includeMusical: boolean): string {
...
function resolveCommon(state: any, opts: any) {
...
function commonMeta(state: any, r: any) {
  return {
    ...
    loras: (state.loras || []).filter((l: any) => l?.name && l.name !== "none" && l.enabled !== false),
...
function buildMiniMaxGraph(state: any, opts: any) {
...
function buildAceStepGraph(state: any, opts: any) {
  ...
  const stages: any[] = [];
  allStages.forEach((st: any, i: number) => {
