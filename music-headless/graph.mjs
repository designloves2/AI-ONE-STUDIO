// graph.mjs — MusicMaker ONE STUDIO (TJ) workflow graph builders, headless.
// 1:1 port of ComfyUI-TJ_NODE_STUDIO_ONE/web/music/graph_builder_music.js
// (== src/tools/music/graphBuilder.ts, minus types). Node ids, class_type, input
// field names, and the dotted key "format.quality" match the studio exactly.
...
function resolveCommon(state, opts) {
  ...
  if (!caption) throw new Error("caption is empty — the headless caller must supply a finished style caption");
...
function commonMeta(state, r) {
  return {
    ...
    caption: r.caption, captionBrief: state.captionBrief || "",
...
function buildMiniMaxGraph(state, opts) {
  ...
  const vae  = state.dav  || "";
  if (!unet || !clip || !vae) throw new Error("MiniMax Music models missing (dit / clip / dav) — set them in job.json or the studio config.");
...
// ── engine: Ace-Step 1.5 (3× progressive SamplerCustom) ─────────────────────
function buildAceStepGraph(state, opts) {
  ...
  if (!unet || !clip1 || !clip2 || !vae)
    throw new Error("Ace-Step models missing (aceUnet / aceClip1 / aceClip2 / aceVae).");
  const r = resolveCommon(state, opts);
  const P = "ACE";
