// view.ts — Qwen Image Edit 2511 메인 화면 조립. Klein의 레이아웃(450px 좌측 패널, 180px 프롬프트,
// Send-to/Compare/Reset/Unload/Settings/Gallery/Help 패턴)을 그대로 따르되, KV Cache 버튼은 없고
// 대신 7개 모드(T2I/I2I/EDIT/PAINT/FACESWAP/ANGLE/UPSCALE)와 ANGLE 모드의 3D 카메라 컨트롤을 추가한다.
import type { QEState, QEMode, PaintSubMode } from "./core";
import {
...
export function renderQwen2511(root: HTMLElement) {
  clear(root);
  root.className = "flex-1 min-h-0 flex flex-col";
  root.style.background = C.bg0;

  const state: QEState = defaultState(loadState());
  ...
  function currentSourceFilename(): string {
    ...
    if (state.mode === "inpaint") return (state.paintSubMode === "outpaint" ? state.inpaintImage : state.inpaintImage) || "";
    if (state.mode === "faceswap") return state.faceswapTarget || "";
    if (state.mode === "angle") return state.angleCameraImage || "";
  ...
  const settingsOv = createSettingsOverlay(state, {
    ...
    onLightningChange: () => { if (state.mode === "t2i" || state.mode === "i2i" || state.mode === "edit" || state.mode === "inpaint" || state.mode === "faceswap" || state.mode === "angle") renderLeftPanel(); },
  } as any);
  ...
  function loraSection(loras: () => QEState["loras"]) {
  ...
  // Faceswap 전용 단일 LoRA 슬롯 (일반 LoRA와 독립, 최대 1개)
  function bfsLoraSection() {
    ...
    function rebuild() {
      ...
      if (lora) {
        ...
        wrap.appendChild(button("+ Add Face LoRA", () => { state.bfsLora = { name: "none", strength: 1, enabled: true }; persist(); rebuild(); }));
      }
    }
    rebuild();
    return wrap;
  ...
  function angleSection() {
    ...
    fileIn.addEventListener("change", () => { if (fileIn.files?.[0]) handleFile(fileIn.files[0]); });
    uploadWrap.addEventListener("dragover", (e) => { e.preventDefault(); uploadWrap.style.borderColor = BRAND; });
    uploadWrap.addEventListener("dragleave", () => { uploadWrap.style.borderColor = C.border; });
    uploadWrap.addEventListener("drop", (e) => { e.preventDefault(); uploadWrap.style.borderColor = C.border; const f = e.dataTransfer?.files?.[0]; if (f) handleFile(f); });
    wrap.appendChild(panel([label("Subject Image"), uploadWrap]));

    const promptPreview = el("div", { style: { fontSize: "11px", color: C.muted, fontFamily: "monospace", padding: "6px 8px", background: C.bg2, borderRadius: "6px", wordBreak: "break-all" } });
    function updatePromptPreview() {
      promptPreview.textContent = buildAnglePrompt(state.angleHorizontal, state.angleVertical, state.angleZoom);
    }

    const scene = createAngleScene(state, () => subjectImg, (axis, val) => {
      if (axis === "h") state.angleHorizontal = val;
      else if (axis === "v") state.angleVertical = val;
      else state.angleZoom = val;
      persist();
      scene.draw();
      updatePromptPreview();
    });
    function loadSubjectPreview(name: string) {
      const im = new Image();
      im.onload = () => { subjectImg = im; scene.draw(); };
      im.src = api.viewUrl(name, "", "input", Date.now());
    }
    if (state.angleCameraImage) loadSubjectPreview(state.angleCameraImage);

    const canvasWrap = el("div", { style: { display: "flex", justifyContent: "center", padding: "8px", background: C.bg1, borderRadius: "8px", border: `1px solid ${C.border}` } });
    canvasWrap.appendChild(scene.el);
    wrap.appendChild(canvasWrap);

    const legend = el("div", { style: { display: "flex", gap: "10px", justifyContent: "center", fontSize: "10px", color: C.muted } }, [
      el("span", { text: "● H (azimuth)", style: { color: "#e91e8c" } }),
      el("span", { text: "● V (elevation)", style: { color: "#00e5c8" } }),
      el("span", { text: "● Z (zoom)", style: { color: "#ffd740" } }),
    ]);
    wrap.appendChild(legend);

    const hSel = select(ANGLE_H_OPTS.map((o) => ({ value: String(o.value), label: o.label })), String(state.angleHorizontal), (v) => { state.angleHorizontal = Number(v); persist(); scene.draw(); updatePromptPreview(); });
    const vSel = select(ANGLE_V_OPTS.map((o) => ({ value: String(o.value), label: o.label })), String(state.angleVertical), (v) => { state.angleVertical = Number(v); persist(); scene.draw(); updatePromptPreview(); });
    const zSel = select(ANGLE_Z_OPTS.map((o) => ({ value: String(o.value), label: o.label })), String(state.angleZoom), (v) => { state.angleZoom = Number(v); persist(); scene.draw(); updatePromptPreview(); });
    wrap.appendChild(panel([
      label("Presets"),
      row([col([label("H"), hSel]), col([label("V"), vSel]), col([label("Z"), zSel])]),
  ...
  function renderLeftPanel() {
    ...
    if (state.mode === "t2i") {
      ...
      leftScroll.appendChild(panel([label("Ref Image 2"), imageUploadSlot(state.editImage2, (name) => { state.editImage2 = name; persist(); })]));
      state.editRefImages.forEach((imgState, i) => {
        ...
        const card = el("div", { style: { position: "relative" } }, [
          removeBtn,
          panel([label(`Ref Image ${i + 3}`), imageUploadSlot(imgState.filename, (name) => { imgState.filename = name; persist(); })]),
        ]);
        leftScroll.appendChild(card);
      });
      if (state.editRefImages.length < MAX_EDIT_REFS - 2) {
      ...
      leftScroll.appendChild(panel([
        label("Source Image"),
        imageUploadSlot(state.inpaintImage, (name) => { state.inpaintImage = name; state.inpaintMaskImage = null; persist(); if (state.paintSubMode === "inpaint") editor?.loadSourceImage(name); }),
      ]));
